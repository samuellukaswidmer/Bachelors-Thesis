...
def extract_prefixes_fixed_length(
    ...
    for case_id, group in event_log.groupby(case_id_col):
        ...
        for i in range(1, prefix_length + 1):
            ...
            features['target'] = target_events.iloc[0]['relative_log_time']
            all_prefixes.append(features)

    return pd.DataFrame(all_prefixes)

def extract_prefixes_multiple_time_thresholds(
    event_log,
    thresholds_days,
    target_activity,
    holidays,
    ...
    for case_id, group in event_log.groupby(case_id_col):
        ...
        for threshold in thresholds_days:
            ...
            if len(prefix) == 0:
                continue
            if target_time <= threshold_cutoff:
                continue
            ...
            if activity_pairs:
                add_pairwise_delays(prefix, features, activity_pairs, activity_col, timestamp_col)
            features['meta::time_threshold_days'] = threshold
